...
public class TCBlockEvent implements Listener {
	...
	public void tntControlBlockDamage(BlockDamageEvent event) {
		...
		if (!event.isCancelled() && event.getBlock().getType() == Material.TNT) {
			...
			Location damagedTNTLocation = new Location(world, x, y, z);
			tntControlLogger.debug("Location of DamagedTNT", damagedTNTLocation);
			tntControlHelper.addTNTToDamaged(damagedTNTLocation);
		}
